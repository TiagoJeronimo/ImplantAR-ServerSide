...
namespace RuntimeGizmos
{
	...
	public class TransformGizmo : MonoBehaviour
	{
		...
		{
            RaycastHit[] hits;
            hits = Physics.RaycastAll(myCamera.ScreenPointToRay(Input.mousePosition), 200.0F);
            ...
            for (int i = 0; i < hits.Length; i++)
            {
                ...
                {
                    if (Input.GetMouseButtonDown(0)) type = TransformType.Move;
                    else if (Input.GetMouseButtonDown(1)) type = TransformType.Rotate;
                }
            }
		...
		void GetTarget()
		{
			if(selectedAxis == Axis.None && (Input.GetMouseButtonDown(0) || Input.GetMouseButtonDown(1)))
			{
